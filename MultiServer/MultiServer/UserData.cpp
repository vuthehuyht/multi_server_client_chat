...
std::vector<UserInformation> UserData::getUserData() {
	loadUserData();
	return userData;
}

std::vector<std::string> UserData::splitArrayOfChar(char message[2048], std::vector<std::string> abc) {
	char* p;
...
bool UserData::checkExistedUser(std::string user) {
	for (std::vector<UserInformation>::iterator i = userData.begin(); i != userData.end(); i++)
...
void UserData::saveNewUser(char message[], SOCKET s) {
...
void UserData::giveModUser(std::string username, std::vector<UserInformation> userData) {
	for (std::vector<UserInformation>::iterator i = userData.begin(); i != userData.end(); i++) {
